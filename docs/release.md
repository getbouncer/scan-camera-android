...
1. run:
    ```bash
    github_changelog_generator -u getbouncer -p cardscan-camera-android -t `cat github_token`
    ```
...
* execute `./gradlew cardscan-camera:install`
...
1. build and upload the library
    ...
    ./gradlew cardscan-camera:install
    ./gradlew cardscan-camera:bintrayUpload
    ```
