...
class Camera2Adapter(
    ...
    private val cameraTexture: TextureView?
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(camera: CameraDevice) {
            ...
            val task = onInitializedFlashTask
            if (task != null) {
    ...
    fun onResume() {
        ...
        if (cameraTexture?.isAvailable != false) {
            openCamera()
        } else {
        ...
        try {
            getAvailableCameras().firstOrNull()?.also { cameraDetails ->
                ...
                if (onImageAvailableListener != null) {
                    onImageAvailableListener.rotationDegrees =
                        calculateImageRotationDegrees(displayRotation, sensorRotation)
                    ...
                    imageReader = ImageReader.newInstance(
                        ...
                        previewFormat,
                        2
                    ).apply { setOnImageAvailableListener(onImageAvailableListener, cameraHandler) }
                }
    ...
    private fun openCamera() {
        ...
        if (cameraTexture != null) {
            configureTransform(Size(cameraTexture.width, cameraTexture.height), previewSize)
        }
